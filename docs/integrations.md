...
One recipe per identity provider: what its tokens contain, the claim mapping
that turns them into a Kubernetes identity, and the RBAC that binds it. Every
recipe is one entry for the `jwt:` list of an
[`AuthenticationConfiguration`](./multi-issuer.md); combine them freely in a
single configuration, and give every issuer its own distinct username and
groups prefix (see
[security: distinct per-issuer prefixes](./multi-issuer.md#security-always-use-distinct-per-issuer-prefixes)).

The same three steps apply to every provider:

1. **Decode a real token** before writing anything; the claims are the raw
   material and their names are not always what they suggest.
2. **Map** a username, groups and, for audit, `extra` values; **validate**
   what must be true of every token, pinning numeric IDs rather than names
   wherever the provider has them.
3. **Bind** roles to the groups the mapping produces, then verify with
   `kubectl auth whoami` and one allowed and one denied action, as in
   [getting started](./getting-started.md#verify-the-first-identity).
...
- [TeamCity (teamcity-oidc-jwt plugin)](#teamcity-teamcity-oidc-jwt-plugin)
- [Google service accounts (workloads on GKE / GCE)](#google-service-accounts-workloads-on-gke--gce)
- [GKE workloads via the cluster's own issuer (per-ServiceAccount identity)](#gke-workloads-via-the-clusters-own-issuer-per-serviceaccount-identity)
...
- [Authentication and identity](./authentication.md) — choosing a
  configuration, the impersonation model, glossary.
- [Multi-issuer authentication](./multi-issuer.md) — the configuration format
  these recipes plug into, readiness and Helm wiring.
- [Auditing](./auditing.md) — what the `extra` values mapped here look like in
  the audit trail.
