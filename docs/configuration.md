...
| `--metrics-bind-address` | — (disabled) | `host:port` of the Prometheus metrics listener. Plain HTTP, unauthenticated, `GET` and `HEAD /metrics` only: any other method on that path is `405` with an `Allow` header and every other path is `404`, so there is no pprof, no index page and nothing to reset. Empty or `0` disables it; port `0` (`:0`) is rejected, because the chart and a ServiceMonitor must know the port. Rendered by the chart when either `metrics.enabled` or `metrics.serviceMonitor.enabled` is true, using `metrics.port`; in that case `extraArgs` may not carry the same flag. With both switches false, `extraArgs.metrics-bind-address` remains available for manually managed listeners. Must differ from `--secure-port` and `--readiness-probe-port`. Diverges deliberately from `--readiness-probe-port`'s port-only shape: an address expresses "disabled" and a loopback bind in one value. See the [metrics reference](./metrics.md) for the catalogue, the label vocabularies and what the endpoint costs, and [chart values `metrics.*`](../chart/kube-oidc-proxy/README.md#metrics) for the Service, the monitors and the NetworkPolicy the chart renders. |
