...
| `--metrics-bind-address` | — (disabled) | `host:port` of the Prometheus metrics listener. Plain HTTP, unauthenticated, `GET` and `HEAD /metrics` only: any other method on that path is `405` with an `Allow` header and every other path is `404`, so there is no pprof, no index page and nothing to reset. Empty or `0` disables it; port `0` (`:0`) is rejected, because a scrape target has to know the port in advance. Must differ from `--secure-port` and `--readiness-probe-port`. Diverges deliberately from `--readiness-probe-port`'s port-only shape: an address expresses "disabled" and a loopback bind in one value. Every family of the catalogue is registered from this phase, but only the build and request families carry samples yet; the authentication, authorization, review-cache, readiness and audit call sites, and the generated metric reference, land with the contract. Hardening is yours to write: bind `127.0.0.1:<port>` when a sidecar does the scraping, keep the port off any Service that reaches beyond the cluster, and restrict who may dial it with a NetworkPolicy for the workload. |
