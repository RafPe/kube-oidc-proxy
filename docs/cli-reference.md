...
| `--subject-access-review-timeout` | `5s` | Timeout authorizing a request's impersonation via SubjectAccessReviews. A single shared budget across all SAR calls for one request (not per-call). Must be greater than 0. |
