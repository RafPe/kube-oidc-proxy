...
2. **Resolve the impersonation target.** If the inbound request also carries
   ...
   identity before honouring it. Decisions are served from a bounded in-memory
   cache before a live `SubjectAccessReview` is issued: allowed and denied
   decisions are cached under separate TTLs
